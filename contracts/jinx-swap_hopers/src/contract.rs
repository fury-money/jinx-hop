...
pub fn execute_swap(
    ...
    //Token1 is always jinx token so if the input is Token1, we should burn some percent of input token
    ...
    match input_token_enum {
        TokenSelect::Token1 => {
            ...
            // burn fee  = 2/1000 * input_amount(the input token in our main token jinx)
        ...
        TokenSelect::Token2 => {
            ...
            // we get burn_fee_amount after swap the other token as jinx
            burn_fee_amount = get_percent_amount(
                token_bought,
                burn_fee_percent_numerator,
                fees.fee_percent_denominator,
